...
16. **不可逆操作沙箱**——agent 默认无生产凭据。任何 DB migration / 真实三方 API 调用 / 部署命令 / 付费操作必须满足：(a) 显式 allowlist 命中、(b) dry-run 先跑且产 Artifact、(c) 强制人审 Decision、(d) 凭据走环境隔离（worktree 内独立 .env，不继承主进程）。git-guard 之外加 `prod-guard.mjs` 守住第二道。**v0.3 落地状态见 [EVA_MULTI_PROJECT_USAGE.md §5 P0-2](proposals/EVA_MULTI_PROJECT_USAGE.md)；prod-guard 实施前必须做 1 天 hook deny 优先级 spike（Claude CLI 多 hook per matcher 合并语义未文档化，详见 R8.11）；用户对 spike fallback 路径拍板 U3-defer（spike 跑完看结果再决定），spike 失败时默认走 U3-B（permission-hook 内置黑名单 + fail-closed）**。
17. **资源隔离原则**——每个 Issue 有专属 worktree、专属 sessionId 命名空间、专属临时文件目录、专属 logical port range。多 agent 并行时不允许共享可写资源。新增 `resource-lock.ts` 模块用 file-lock + DB 行锁双重防护。**v0.3 扩展**：(a) 凭据维度 — cli-runner spawn env 白名单 + injectEnv denylist 对称安全模型（[EVA_MULTI_PROJECT_USAGE.md §5 P0-1](proposals/EVA_MULTI_PROJECT_USAGE.md)，与 [WORKTREE_LOCK.md L41](../WORKTREE_LOCK.md) H13 hook env strip 同源）；(b) cli-runner.ts 单点 — K10 不变量要求 P0-1/P0-2 与 M2 loop2+ 改 cli-runner.ts spawn/env/args/hook 链路必须串行合并，worktree 隔离不能消除同一核心路径的语义冲突（详见 R8.13）。
...
23. **跨端 wire enum 必须 graceful unknown-value fallback**（v0.3 2026-05-05 引入，源自 [EVA_MULTI_PROJECT_USAGE.md K12](proposals/EVA_MULTI_PROJECT_USAGE.md)）——所有 Swift Codable enum 必须实现 `init(from:)` 自定义 decoding，unknown 值 fallback 到 default；所有 TS Zod enum 必须用 `z.enum([...]).catch(default)`；fixture round-trip 测试必含 `'future-unknown-value'` 反例。理由：Swift Codable 默认对 unknown enum case 抛 `DecodingError.dataCorrupted` → 老 iOS 装包遇 backend 新加 enum 值整 DTO decode 失败 → 整个 project 列表显示空白（R8.12 实证）。**这条与 #1 iOS thin shell + #9 server-driven 配套**：server schema 加新 enum 值不是 contract break 的前提就是老客户端能 graceful fallback。详见 [HARNESS_RISKS.md R8.12](HARNESS_RISKS.md)。
...
1. 本里程碑的数据模型（新增 / 修改的实体 + DDL + migration）是否明确，**且 migration 在 prod-shape fixture 上验证过**？"prod-shape" 至少包含：父表有数据、子表有 FK 引用、已有 enum / CHECK / index、真实 user_version 从旧版本升级。空 fixture 测过 ≠ prod 验证过——v0.4.4 H14 prod 失败的根因正是 dev/test fixture 都没有 child rows，掩盖了 schema-level FK 检查会阻塞 DROP TABLE 这一事实（详见 [docs/retrospectives/M2-h14-prod-migration-failure.md](retrospectives/M2-h14-prod-migration-failure.md)）。
