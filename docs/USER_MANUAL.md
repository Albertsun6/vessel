...
项目列表**顶部**始终钉着"💬 随手问"。这是一个不与任何 cwd 绑定的对话容器，适合临时提问或不知道选哪个项目时直接开问。无需配置；后端自动创建并置顶（`sticky: true`），无法删除。
...
7. **Whisper 尾部幻觉过滤**：`voice.ts` 自动剥离 Whisper 在寂静/噪声尾部常见的幻觉片段（如"优优独播剧场——YoYo Television Series Exclusive"），不会混入转写结果。
...
```bash
git tag v0.3.1                          # 锁定版本
./scripts/promote.sh v0.3.1             # prod worktree checkout 新 tag + restart :3030
./scripts/rollback.sh v0.3.0            # 回退到旧 tag（同 promote，传旧 tag）
```

promote 后 :3030 在 5 秒内重启，脚本自动 health check 验证。

### launchd 自启
- Stable plist：`~/Library/LaunchAgents/com.claude-web.backend.plist`（`:3030`）
- Dev plist：`~/Library/LaunchAgents/com.claude-web.backend.dev.plist`（`:3031`）
...
launchctl unload ~/Library/LaunchAgents/com.claude-web.backend.plist   # 关 stable
launchctl load -w ~/Library/LaunchAgents/com.claude-web.backend.plist  # 开 stable
```
...
### Tailscale HTTPS
```bash
tailscale serve --bg --https=443 http://localhost:3030   # prod → 443
tailscale serve --bg --https=8443 http://localhost:3031  # dev → 8443
```
得到 `https://<hostname>.<tailnet>.ts.net`（prod）或 `:8443`（dev），跨设备 HTTPS 访问。
...
### 快速切换后端（Settings 快捷按钮）

Settings → Backend 区块底部有 4 个一键切换按钮：

| 按钮 | URL | 场景 |
|---|---|---|
| Prod — 模拟器 | `http://localhost:3030` | 模拟器连 stable backend |
| Prod — Tailscale | `https://<host>.ts.net` | 真机连 stable backend |
| Dev — 模拟器 | `http://localhost:3031` | 模拟器连 dev backend |
| Dev — Tailscale (:8443) | `https://<host>.ts.net:8443` | 真机连 dev backend |

点后立即生效（URL 写入并触发 WS 重连），底部脚注说明 Prod 用已发布代码，Dev 用开发中代码。
