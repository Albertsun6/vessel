...
### P5. 结构化审视 skill（架构存量交叉一致性扫描）
灵感：现有 [`harness-architecture-review`](.claude/skills/harness-architecture-review/SKILL.md) 是单 artifact 评审，[`reviewer-cross`](.claude/skills/reviewer-cross/SKILL.md) 是单契约评审，都是"对象绑定"。本 skill 是**存量绑定**——对当前 harness 文档全集 + `.claude/skills/` 目录做 4 类一致性扫描，找出已被各处局部决策淹没的全局矛盾。

**扫描 4 类问题**：
1. 内部冲突（A 文档说 X，B 文档说非 X）
2. 已过时（决策已被新约束覆盖但旧文未删）
3. 重复定义（同一概念在多处独立定义且语义漂移）
4. skill description 触发碰撞（≥2 个 SKILL.md 描述会被同一 prompt 触发）

**触发模式**：
- **manual-only**：用户输入 `/architecture-audit`，**无自动 fire**（自动 fire 留 [HARNESS_ROADMAP.md §17 Q15](docs/HARNESS_ROADMAP.md)）。
- 单次执行：fresh context，**不读作者 transcript / 任何先前评审**（同 reviewer-cross 独立性约束）。
- 输入 cap ≤80k tokens（按层切片喂入：先 L1-L3，再 L4-L6，再 L7 横切；超 cap 自动分轮）。

**输出**：
- 单份 findings 报告写到 `docs/audits/<YYYY-MM-DD>-architecture-audit.md`。
- 每条 finding 自动产一条 `IdeaCapture`，POST 到 `/api/inbox`（已就绪），与 §16 三路径合流。
- **不写 PR / 不直接改 methodology / 不改 skill 文件**——硬约束，违反 [HARNESS_ROADMAP.md §16.3 第 1 条](docs/HARNESS_ROADMAP.md)。

**实现位置**：
- `.claude/skills/architecture-audit/SKILL.md`（新建）
- 触发命令：claude-web 现有 slash-command 机制（参考 [docs/USER_MANUAL.md](docs/USER_MANUAL.md) 中 skill 触发段）。

**为什么值得**：M-2/M-3 累积 methodology 版本 + skill 之后，全局矛盾会肉眼难发现。artifact-bound review 抓不到"存量在一起还自洽吗"。

**为什么不优先**：
- 准入条件：`.claude/skills/` ≥ 10 个 **且** methodology v1→v2 至少升过 1 次。
- M-2 还没真实存量，提前做就是"审空数据库"。

**风险**：
- 单次烧 token：cap + 按层切片 + hash-cache 命中跳过。
- 与 architecture-review 边界混淆：在 SKILL.md 第一行就写"扫存量、不审单 artifact"。
- findings 沦为噪声：每条必须挂可执行的 IdeaCapture，否则不输出。

---

### P6. Web 端 inbox UI（capture + list + triage）
iOS 已有完整 inbox（[InboxCaptureSheet.swift](packages/ios-native/Sources/ClaudeWeb/Views/InboxCaptureSheet.swift) + [InboxListView.swift](packages/ios-native/Sources/ClaudeWeb/Views/InboxListView.swift)），Web 端 0 UI——只能 `curl POST /api/inbox`。

**实现**：
- 浮动"💡"按钮 → 弹 modal 输入框 POST `/api/inbox`
- 侧边栏新增 "Inbox" 抽屉显示列表 + triage 按钮（与 iOS swipe actions 对齐：派给 Claude / 分到 IDEAS / 归档）
- 复用 [packages/frontend/src/auth.ts authFetch](packages/frontend/src/auth.ts) 走鉴权
- 复用 backend 已有 [POST /api/inbox/:id/triage](packages/backend/src/routes/inbox.ts)（C2 上线后）

**不做**：编辑、批量操作、过滤器持久化。

**准入条件**：backend C1+C2 + iOS C3 都跑通后再开（确保 triage 端点稳定再做 web 客户端）。

**为什么值得**：在桌面前的时候手机不在身边，想存一笔得切到手机。Web 端最小 UI 能补上。

**为什么不优先**：iOS 已经能 cover 主要"随手记"场景；桌面记笔记可以复用 IDE / 备忘录；只在确实想用 web 的时候才有用。
...
### H18. Provider Runtime Matrix（Claude / Cursor / Codex） ⭐⭐
灵感来自 Multica 的 provider 能力矩阵 + 当前 Eva 已经用 cursor-agent 做 cross review 的事实：Claude CLI、Cursor CLI、Codex CLI 都是“订阅账号 + 本地/官方 CLI agent”，都带一部分基础 harness 能力（session、模式、工具、规则、sandbox/cloud 等）。Eva 不应永远把底层叫 `ClaudeRunner`，但也不应过早做通用多 provider 平台。

**可借鉴点**：
- 把 `claude` / `cursor-agent` / `codex` 的能力显式列成 `ProviderCapabilities`，而不是散落在 prompt 和代码里。
- Claude CLI 继续作为 primary executor；Cursor / Codex 先作为 reviewer-only runtime，验证 review 质量后再考虑执行写代码。
- 所有 provider 输出必须归一化为 Eva 自己的 `RunEvent`，不要让 iOS/Web 直接理解各 CLI 的私有事件格式。
- 用 provider matrix 驱动 UI/调度：支持 resume、streaming JSON、permission hook、MCP、plan/ask、worktree、cloud task、image input 等能力时才显示对应功能。

**适用性**：
- 用户价值：4（复用多家订阅 + 多模型交叉评审）
- 架构贴合：4（Eva 已经是 CLI subprocess harness；但当前 `cli-runner.ts` 与 Claude 深耦合）
- 实现复杂度：3（matrix 本身低，真正 adapter 中高）
- 风险：3（抽象过早会拖慢 Claude 主链路；不同 CLI 的权限/输出/session 语义不一致）
- 优先级：**P2**（先做 capability matrix + reviewer-only spike，不抢 H12/H13/H14）

**实现草案**：
1. `packages/backend/src/providers/capabilities.ts`：只定义静态能力矩阵，不改执行路径。
2. `RunRecord` / retrospective 记录 `provider: "claude"`，先为未来兼容留字段。
3. Cursor/Codex reviewer spike：输入 `diff + spec + test log`，输出统一 review markdown（BLOCKER/MAJOR/MINOR）。
4. 抽 `ProviderAdapter` 之前必须完成 spike checklist：非交互调用、输出格式、resume、interrupt、cwd/worktree、权限/sandbox、失败码、订阅限额。
5. 真正多 provider 执行器只在 Cursor/Codex review 质量稳定后进入 M3 设计。

**不做**：
- 不把 Claude CLI 替换掉；Claude 仍是默认 runtime。
- 不让 Cursor/Codex 直接进入 iOS/Web 协议。
- 不为了多 provider 引入 Anthropic Agent SDK 或 OpenAI API key 计费路径。
- 不做“几个 AI 互相聊天”的无限 review loop；review prompt 必须只审 bug / 安全 / 行为回归 / 缺测试 / 契约错配。

**参考**：
- Cursor CLI：agent / plan / ask、rules、MCP/ACP、sessions、sandbox、worktree/cloud handoff。
- Codex CLI：ChatGPT 订阅登录、本地 coding agent、code review、web search、cloud tasks。
- Claude CLI：stream-json、permission hook、resume、MCP、当前 Eva 主链路。

---

## slopus/happy iOS 端对照（2026-05-14）

[slopus/happy](https://github.com/slopus/happy) 是 Expo React Native 一套代码出 iOS+Android+Web+Tauri 桌面的 Claude Code / Codex 移动客户端，已上 App Store / Play Store。和 Seaidea 定位高度重叠，但架构选择正好相反——做了一次对照分析做为借鉴 vs 拒绝的判断依据。

**仓库快照**：`/tmp/happy-analysis`（shallow clone，本机临时）；规模 89,396 行 .ts/.tsx + 168 npm deps（Seaidea 15,642 行 .swift + ~20 deps）。

### 架构根本差异

| | happy | Seaidea |
|---|---|---|
| 框架 | Expo 55 + RN 0.83 + TS（多平台） | SwiftUI + Swift 5.9（iOS only） |
| 后端 | `api.cluster-fluster.com` 中转（第三方运营） | 用户自己 Mac 的 `:3032` backend |
| 加密 | libsodium 端到端（PBKDF2 派生 + AES + HMAC-SHA512） | 网络层 Tailscale WireGuard |
| 配对 | QR 码 + challenge/response + secret key 备份 | mDNS `_vessel._tcp` + 手动 IP 兜底 |
| 配置更新 | build-time + Expo OTA | server-driven config（`/api/harness/config` + ETag） |
| 遥测 | PostHog SaaS | 自建 `/api/telemetry` + jsonl 落盘 |
| 多 agent | Claude / Codex / Gemini / 通用 ACP | Claude only |

### happy 多出的能力

| 能力 | 实现栈 | 价值 |
|---|---|---|
| 实时双向语音 | LiveKit 2.15 + WebRTC + ElevenLabs agent + VAD（0.5 阈值/300ms 静音） | 真双向，不是 STT→LLM→TTS 串行 |
| 推送通知 | APNs + FCM + expo-notifications + 前台抑制 | PreToolUse 等待时手机弹推送 |
| 代码 diff + 语法高亮 | CodeMirror 6 + @pierre/diffs + 12+ 语言 | 文件改动可视化 |
| Sandbox 配置 UI | `happy sandbox` 子命令配置进程隔离 | 用户面 trust boundary |
| Daemon 远程派发 | 离开电脑能在手机创建新 session | 真"出门也能让 agent 干活" |
| Figma 主题工程 | `theme.gen.ts` + theme.figma.json | 设计师工作流 |

### Seaidea 多出的能力

- **Server-driven config**：改 copy / model / feature flag 不重装 app（happy 全 build-time）
- **mDNS 自动发现**：插上 Mac 自动连，不用手动输 server URL
- **目录选择器 + mkdir**：[DirectoryPicker.swift](packages/ios-native/Sources/ClaudeWeb/DirectoryPicker.swift) breadcrumb 导航
- **每会话 TTS 缓存 + 重播**：[TTSPlayer.swift](packages/ios-native/Sources/ClaudeWeb/TTSPlayer.swift) 切会话不丢音频
- **个人遥测无 SaaS**：[Telemetry.swift](packages/ios-native/Sources/ClaudeWeb/Telemetry.swift) → 用户自己 backend
- **跨设备项目注册表**：`/api/projects` server-of-truth

### 借鉴 vs 拒绝决策表

| happy 能力 | 决策 | 理由 |
|---|---|---|
| WebRTC 实时语音 | ❌ 拒绝 | 个人助理 overkill；edge-tts 分句串流已够用；ElevenLabs 收费 + 数据出境违反个人单机原则 |
| 推送通知（APNs/FCM） | ⚠️ M2 缓议 | PreToolUse 授权等待时确实有用；要做必须走自建 APNs，不接第三方推送 |
| 多 agent（codex/gemini/acp） | ⚠️ M2+ 考虑 | 改 [cli-runner.ts](packages/backend/src/cli-runner.ts) + iOS 选择器；当前 claude 订阅够用 |
| **代码 diff + 语法高亮** | ✅ M2 借鉴 | 文件 diff 视图明显有用；用 SwiftUI 原生（Highlightr 或自写），不引 CodeMirror |
| Sandbox 配置 UI | ✅ M3+ 借鉴 | trust boundary 模型可抄 |
| **Daemon 远程派发** | ⚠️ M2 借鉴 | "手机上创建新 session"是真需求；当前 Seaidea 只能续已开的 |
| QR 码配对 | ❌ 拒绝 | mDNS + Tailscale 已解决；加 QR 是多余 UX + 摄像头权限 |
| Figma 主题工程 | ❌ 拒绝 | 单人项目，SwiftUI 颜色系统够用 |
| libsodium E2E 加密 | ⚠️ M3 评估 | Tailscale 已加密；除非将来出 Tailscale 走公网否则不必 |
| OTA 热更新 | ❌ 与栈无关 | SwiftUI 没法 OTA；server-driven config 已是等效路径 |

### 战略结论

**Vessel 不变式视角**——Vessel CLAUDE.md 4 条核心约束：(1) 个人单机不走第三方中转、(2) 订阅认证不走 API SDK、(3) server-driven config 不重装、(4) iOS 优先 SwiftUI。

**happy 违反 #1 #3 #4**：cluster-fluster.com 中转是其根架构非可选，PostHog 上报违反 #1 精神，build-time 配置违反 #3，Expo RN 违反 #4（除非重开方向决策）。

**happy 与 Vessel 兼容**：(a) 都走 CLI 子进程认证（happy 实测会 spawn `claude`，不走 SDK），(b) 都有 bearer token 鉴权，(c) E2E 加密对 Vessel 是可选 future-proof。

**实操优先级（按价值 / 工作量排序）**：
1. **M2**：代码 diff 视图（高价值，~1 周，纯 iOS）
2. **M2**：daemon 远程派发（中价值，需 backend 改 + iOS UI，~2 周）
3. **M2 缓议**：推送通知（自建 APNs，~1 周）
4. **M3+**：sandbox 配置 UI（趋势性需求）
5. **跳过**：QR 配对 / E2E 加密 / 多 agent / Figma 主题 / WebRTC 语音

**为什么记下来**：happy 是当前 GitHub 上最完整的"Claude Code 手机端"参考实现，已上架商店。它做了的能力 Vessel 不必再调研，可以直接 cherry-pick；它做的架构选择（云中转 + RN + 第三方 SaaS）正好是 Vessel 要拒绝的反面教材——记下来避免将来评审时被"别人都这么做"绕回去。

---

## 多项目使用 H 段（v0.3 新增，源自 EVA_MULTI_PROJECT_USAGE.md）

> **来源**：[docs/proposals/EVA_MULTI_PROJECT_USAGE.md v0.3](proposals/EVA_MULTI_PROJECT_USAGE.md) §7.2 经 phase 1+2+3 评审收敛 + 用户拍板（2026-05-05）。
>
> **触发条件**：M2 master plan loop2+ 期间，用户从 dogfood 走到多 Subject 工程项目落地的相变窗口。

### H19. spawn env 白名单 + injectEnv denylist 对称安全模型 ⭐⭐⭐ (P0 必做)

灵感来自业界 OpenHands [#13268](https://github.com/OpenHands/OpenHands/issues/13268) SDK credential inheritance 显式建模 + Eva 自身 H13 hook env curated（[WORKTREE_LOCK.md L41](../WORKTREE_LOCK.md)）。

**问题**：[cli-runner.ts:80](../packages/backend/src/cli-runner.ts#L80) 当前 `env: process.env` 默认全继承，敏感凭据泄到 spawn 子进程。

**实现要点**：
- AgentProfile 加 `inheritEnv: string[]` 默认白名单 11 项 + `injectEnv?: Record<string, string>` + `inheritMode: "curated" | "full"`
- injectEnv denylist 9 项（PATH/HOME/BASH_ENV/ENV/SHELLOPTS/CDPATH/IFS/PS1/PS4）防反向打洞
- 退出条件三层：negative canary + 30 天 dogfood Run 命令 dry-run + InjectEnv denylist 单元测试

**对应 RISKS**：R8.1 + R8.10。详见 EVA_MULTI_PROJECT_USAGE §5 P0-1。

### H20. prod-guard.mjs + spike 前置 + 黑名单单源真相 ⭐⭐⭐ (P0 必做)

灵感来自 OpenHands [#13506](https://github.com/OpenHands/OpenHands/issues/13506) audit log + agent-scoped identity + Eva [context-manager.ts NEVER_ALLOWED](../packages/backend/src/context-manager.ts#L86-L94)。

**实现要点**：
- 第一步 1 天 spike：验证 Claude CLI hook deny 优先级合并语义
- 黑名单单源真相 = `~/.claude-web/never-allowed-commands.json`（与 ContextManager NEVER_ALLOWED 同源避免漂移）
- Token-level 检测（用 [shell-quote](https://www.npmjs.com/package/shell-quote) npm 包）+ wrapper 解析（`pnpm/npm/yarn run` 必须 ResolveScript 解析 package.json scripts）
- Allowlist override 走 `~/.claude-web/prod-guard-allowlist.json`（用户手动编辑 + audit log）

**对应 RISKS**：R8.2 + R8.11。详见 EVA_MULTI_PROJECT_USAGE §5 P0-2。

### H21. harness_project domain_profile + projects.json ↔ harness_project 同步规则 + 跨端契约 ⭐⭐⭐ (P0 必做)

灵感来自 [Multica provider capability matrix](https://github.com/multica-ai/multica) + 对 Eva 现有 [methodology.applies_to enum](../packages/backend/src/migrations/0001_initial.sql#L108) 概念扩展。

**实现要点**：
- 拆 0004 additive migration（minor v103，加 domain_profile 列）+ 0005 schema-rebuild（major v200，改 methodology.applies_to enum）
- 5 选 enum：`software-enterprise / software-library / software-cli / infra-script / dogfood-self`
- ProjectDTO.domainProfile + Swift Codable `init(from:)` unknown enum fallback + TS Zod `.catch(default)`
- projects.json POST `/api/projects` 时 backend 同步 INSERT harness_project；老 iOS 不传字段 → log warning + default + `needs_user_review` flag
- minClientVersion bump 字段保护老 iOS 装包

**对应 RISKS**：R8.3a + R8.12。详见 EVA_MULTI_PROJECT_USAGE §5 P0-4。

**用户拍板（2026-05-05）**：U1-A 仅保留 5 选粗类型，business_domain 进 PM spec 必填字段不进 schema；U2-C legacy backfill 用 `software-enterprise` + `needs_user_review` flag。

### H22. Scheduler per-project rate limit ⭐⭐ (P1)

**问题**：单 SQLite 单进程 + Scheduler 全局 setInterval，多 active project 并行时 SQLite 写锁竞争。
...
**对应 RISKS**：R8.4。详见 EVA_MULTI_PROJECT_USAGE §5 P1-1。

### H23. iOS 项目维度抽象（拆后端 prefix + iOS UI 两步）⭐⭐ (P1)

**问题**：iOS BackendClient 是 per-conversation 不是 per-project，多项目时 push / Inbox / Decision 混乱。

**实现要点**：
- P1-2a 后端：通知 title 加 `[<projectName>]` prefix；payload 保留 `projectName / projectId` 结构化字段（保护 ServerChan / Telegram webhook）
- P1-2b iOS UI：顶栏项目切换器 + Inbox 分组（**承认破坏 K4 thin shell 假设**）

**对应 RISKS**：R8.5 + R8.14。详见 EVA_MULTI_PROJECT_USAGE §5 P1-2。

### H24. PreToolUse hook deny 优先级 spike (P0-2 前置 1 天) ⭐⭐⭐

**目的**：验证 Claude CLI 多 hook per matcher 时 `permission-hook(allow) + prod-guard(deny)` 是否取最严格语义。

**实现要点**：
- 跑一对冲突测试：故意配两个 hook，permission-hook 回 allow / prod-guard 回 deny，看 CLI 实际取哪个
- 同时跑 hook timeout / hook crash / 网络 fail 三种异常场景
- spike verdict 落 `docs/reviews/p0-2-hook-deny-priority-spike-YYYY-MM-DD.md`
- spike 失败 → 走用户拍板 U3-defer（默认 U3-B = permission-hook 内置黑名单）

**对应 RISKS**：R8.11。详见 EVA_MULTI_PROJECT_USAGE §5 P0-2-spike + U3。

### H25. Swift Codable / TS Zod 跨端 enum graceful fallback 标准 ⭐⭐ (K12 不变量)

**问题**：Swift Codable 默认对 unknown enum case 抛 `DecodingError.dataCorrupted` → 老 iOS 遇 backend 新 enum 整 DTO 失败 → 所有 project 列表空白。

**实现要点**：
- Swift Codable 必须实现 `init(from:)` 自定义 decoding，unknown 值 fallback 到 default
- TS Zod 用 `z.enum([...]).catch(default)` 同样 fallback
- Fixture round-trip 测试加 `'future-unknown-value'` 反例
- 升级为 K12 关键不变量（所有跨端 wire enum 必须有 graceful fallback）

**对应 RISKS**：R8.12。详见 EVA_MULTI_PROJECT_USAGE §6 K12。

### H26. harness.db 三层备份（覆盖范围 + 完整性 + 可恢复性）⭐⭐ (P1)

**问题**：harness.db 单文件，无任何备份机制；只备 .db 漏 artifacts / audit / projects.json / telemetry。

**实现要点**：
- 覆盖范围：备份 `~/.claude-web/` 整目录（含 artifacts/ + audit / projects.json / telemetry / harness.db）
- 完整性：每天备份后跑 `sqlite3 backup.db.snapshot "PRAGMA integrity_check"` + `PRAGMA foreign_key_check`
- 可恢复性：每月 1 号自动 dry-run 恢复演练，失败发 Telegram alert
- 保留 30 天 daily + monthly 永久 snapshot
- `.env.harness` 不混入普通快照（避免明文密钥进备份）

**对应 RISKS**：R8.6 + R8.15 + K11。详见 EVA_MULTI_PROJECT_USAGE §5 P1-3 + P2-3。
...
| H18 Provider runtime matrix | Multica + Cursor/Codex/Claude CLI docs | P2 | provider 能力先显式化；Cursor/Codex 先 reviewer-only |
