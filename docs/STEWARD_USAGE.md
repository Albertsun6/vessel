...
1. 主窗口当前正忙
2. 粘 `开始干 m2-voice-proposal`（一个 docs only 的 task）
3. Claude 看 `parallel_safe_files=["docs/proposals/"]` + 当前主线在改 `packages/backend/` → **没冲突，建议开新窗口**
4. Claude echo 5 步命令模板：
   ```
   ...
   ```
5. 你说 `ok` → Claude 执行
6. 新 Cursor 窗口弹出 → 启 Claude → 粘那条 prompt → 它接着干
...
| `pnpm eva:sessions` | 派生视图（活窗口）| Steward "看下一步" 算法的输入；lazy |
